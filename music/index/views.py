...
def indexView(request):
    ...
    # 热门歌曲
    hot_song = Dynamic.objects.select_related('song').order_by('-dynamic_vote').all()[:3]
    play_hot_song = Dynamic.objects.select_related('song').order_by('-dynamic_vote').all()[:10]
    # 新歌推荐
